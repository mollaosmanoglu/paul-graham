...
-   **Motion `scroll()` / `useScroll` / `useTransform`** with compositor targets — S-tier when mapped to hardware-accelerated outputs (see below)

### Motion `useScroll` / `useTransform` (scroll-linked)

Motion hardware-accelerates scroll-linked animations via WAAPI / ScrollTimeline when they write compositor properties. Whether it stays on the compositor depends on how the scroll progress reaches the value:

**S-tier patterns:**

1. Bind scroll progress **directly** to a compositor value — most commonly `opacity`, whose 0→1 range matches scroll progress:
...
2. Map scroll progress through a **single** `useTransform` in **array-keyframe** form onto a compositor value (`opacity`, `transform` as literal strings, `filter`, `clipPath`):

```tsx
const opacity = useTransform(scrollYProgress, [0, 0.5], [1, 0])
const transform = useTransform(scrollYProgress, [0, 1], ["translateY(0dvh)", "translateY(-24dvh)"])
return <motion.div style={{ opacity, transform }} />
```

**Back to the main thread (A-tier), not S-tier:**

- `useTransform` in **function form** — `useTransform(() => …)`.
- A value passed **more than one `useTransform` deep** — chaining a `useTransform` through another `useTransform`, or through `useMotionTemplate`. Motion only hardware-accelerates a single array-keyframe hop.

**Still A-tier (not scroll-specific):**

- Motion `x` / `y` / `z` **shorthand** in `initial` / `animate` / variants — author literal `transform: "translateY(...)"` / `"translateX(...)"` strings instead.
- `useSpring(useTransform(...))` — spring smoothing reintroduces main-thread interpolation between scroll samples.
...
// S-tier - Motion scroll-linked (hardware accelerated via ScrollTimeline / WAAPI)
const { scrollYProgress } = useScroll({ target: ref })
const opacity = useTransform(scrollYProgress, [0, 1], [1, 0])
const transform = useTransform(scrollYProgress, [0, 1], ["translateY(0px)", "translateY(-120px)"])
<motion.div style={{ opacity, transform }} />
